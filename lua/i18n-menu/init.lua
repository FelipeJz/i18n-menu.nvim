...
function M.show_buffer_keys_menu()
  ...
  if not messages_dir or not translation_files then return end
...
function M.show_all_keys_menu()
  ...
  local sorted_keys = {}
  for key in pairs(keys_set) do table.insert(sorted_keys, key) end
...
function M.setup()
  ...
  api.nvim_command(
    "autocmd InsertLeave *.jsx,*.tsx lua require('i18n-menu.util').highlight_translation_references()")
